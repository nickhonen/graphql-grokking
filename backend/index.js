...
const resolvers = {
  ...
  Author: {
    bookCount: (root) => {
      const duplicateAuthors = books.filter(b => b.author === root.name)
      return duplicateAuthors.length
    }
  },
  Mutation: {
    addBook: (root, args) => {
      if (books.find(b => b.title === args.title)) {
        throw new GraphQLError('Book already present', {
          extensions: {
            code: 'BAD_USER_INPUT',
      ...
      if (!(authors.find(a => a.name === args.author))) {
        const newAuth = { name: args.author }
        authors = [...authors, newAuth]
      }
